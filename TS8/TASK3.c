...
int search_key(){
	FILE *key = fopen("keys", "r");
	char s[100];
	char * dataP;
	int not_found=0,attemps,location,number=0,len;
	while(fgets(s,90,key)){
		len= strlen(s)-1;
		s[len] = '\0';
		dataP = malloc(strlen(s)+1);
		assert(dataP != NULL);
		strcpy(dataP, s);
		attemps=0;
		while(1){
			location = indexAdvisedByHash(makeNumber(s), attemps);
			if(table[location]==NULL){
				break;
			}
			else{
				if(table[location]!=NULL&&strcmp(table[location],dataP)==0){
					//printf("%s\n" , dataP);
					number++;
					break;
				}
		...
		while(1){
			for(i=0;i<usedSize;i++){
				if(table[i]==NULL){
					...
					if(table[i]!=NULL&&strcmp(table[i],dataP)==0){
						found++;
						break;
					}
				}
...
int main(void) {
	...
	char strg[100];
	char * dataP;
	int loc, collisions=0, noCollisions=0,max=0,f_n,search,s_search;
