...
void heapifyUp(int index) {
	int par = index/2;
	struct Huff *temp;
	while(heap[par]->freq>=heap[index]->freq){
		swapAtIdx(par,index);
		index = par;
		par = index/2;
		if(par==0){
	...
	// See Figure 12.7 in Reema Thareja textbook
}

void heapifyDown(int parent) {
	int left=2, right=3;
	while((heap[left]->freq<heap[parent]->freq)||(heap[right]->freq<heap[parent]->freq)){
		if(heap[right]->freq<=heap[left]->freq){
			swapAtIdx(parent,right);
			parent=right;
			left=parent*2;
			right=parent*2+1;
			if(left>=heapSz){
