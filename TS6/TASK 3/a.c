...
static struct node * deleteNode(int key, struct node *tree){
	...
	if (tree == NULL)
		...
		assert(tree->key == key);
		//code for deleating node with no child
		if(tree->right==NULL&&tree->left==NULL){
			free(tree);
			return NULL;
		}
	    //code for deleating node with one child
	    else if((tree->right==NULL&&tree->left!=NULL)||(tree->left==NULL&&tree->right!=NULL)){
	    	if(tree->left==NULL){
				free(tree);
				return tree->right;
			}
		...
		//code for deleating node with two childs.
		else{
			int k = graftReplacementKey(tree);
			tree->key = k;
			return tree;
		}
	}
